...
//     const { token, userId, role } = await loginService(username, password, expectedRole);
...
export const loginController = async (c: Context) => {
  try {
    const { username, password, expectedRole } = await c.req.json();
    ...
    if (error instanceof Error) {
      ...
      if (error.message === "Role tidak sesuai") {
